...
struct find_args
{
	struct kgr_kallsyms_reloc reloc;
...
static int (*kgr_module_kallsyms_on_each_symbol)(int (*fn)(void *, const char *,
							   struct module *,
							   unsigned long),
						 void *data);
...
static int __kallsyms_relocs_init(void)
{
	...
	if (kgr_module_kallsyms_on_each_symbol)
		return 0;

	kgr_module_kallsyms_on_each_symbol =
	...
	for (i = 0; i < count; ++i) {
		*relocs[i].addr = NULL;
		...
		if (args.reloc.objname) {
			kgr_module_kallsyms_on_each_symbol(__find_callback,
							   &args);
		} else {
		...
		if (!*relocs[i].addr) {
			if (relocs[i].objname) {
				pr_err("kgraft-patch: symbol %s:%s not resolved\n",

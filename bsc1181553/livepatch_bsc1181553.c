...
#else
static int  __read_mostly (*klpe_futex_cmpxchg_enabled);
...
static int
(*klpe_get_futex_key)(u32 __user *uaddr, int fshared, union futex_key *key, enum futex_access rw);
...
static void (*klpe_wait_for_owner_exiting)(int ret, struct task_struct *exiting);
...
static int klpp_wake_futex_pi(u32 __user *uaddr, u32 uval, struct futex_pi_state *pi_state)
{
	...
	ret = (*klpe_cmpxchg_futex_value_locked)(&curval, uaddr, uval, newval);
	if (!ret && (curval != uval)) {
		/*
...
retry:
	ret = (*klpe_get_futex_key)(uaddr1, flags & FLAGS_SHARED, &key1, FUTEX_READ);
	if (unlikely(ret != 0))
		goto out;
	ret = (*klpe_get_futex_key)(uaddr2, flags & FLAGS_SHARED, &key2, FUTEX_WRITE);
	if (unlikely(ret != 0))
		goto out_put_key1;
...
retry:
	ret = (*klpe_get_futex_key)(uaddr, flags & FLAGS_SHARED, &q.key, FUTEX_WRITE);
	if (unlikely(ret != 0))
		goto out;
...
retry_private:
	...
	ret = (*klpe_futex_lock_pi_atomic)(uaddr, hb, &q.key, &q.pi_state, current,
				   &exiting, 0);
	if (unlikely(ret)) {
		/*
		...
		case -EAGAIN:
			 ...
			 */
			(*klpe_wait_for_owner_exiting)(ret, exiting);
			cond_resched();
			goto retry;
		default:
			goto out_unlock_put_key;
...
retry:
	...
	ret = (*klpe_get_futex_key)(uaddr, flags & FLAGS_SHARED, &key, FUTEX_WRITE);
	if (ret)
		return ret;
	 ...
	 * owner.
	 */
	if ((ret = (*klpe_cmpxchg_futex_value_locked)(&curval, uaddr, uval, 0))) {
		spin_unlock(&hb->lock);
		switch (ret) {
		case -EFAULT:
			goto pi_faulted;
...
static struct klp_kallsyms_reloc klp_funcs[] = {
	...
	{ "cmpxchg_futex_value_locked", (void *)&klpe_cmpxchg_futex_value_locked },
	{ "hash_futex", (void *)&klpe_hash_futex },
	{ "rt_mutex_next_owner", (void *)&klpe_rt_mutex_next_owner },
	{ "__rt_mutex_start_proxy_lock", (void *)&klpe___rt_mutex_start_proxy_lock },
	{ "rt_mutex_wait_proxy_lock", (void *)&klpe_rt_mutex_wait_proxy_lock },
	{ "rt_mutex_cleanup_proxy_lock", (void *)&klpe_rt_mutex_cleanup_proxy_lock },
	...
	{ "wait_for_owner_exiting", (void *)&klpe_wait_for_owner_exiting },

...
static struct hci_chan *(*klpe_hci_chan_create)(struct hci_conn *conn);
...
static void klpr_hci_encrypt_change_evt(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	if (!ev->status && ev->encrypt && conn->type == ACL_LINK) {
		...
		if ((*klpe_hci_send_cmd)(hdev, HCI_OP_READ_ENC_KEY_SIZE,
				 sizeof(cp), &cp)) {
...
static void klpp_hci_user_confirm_request_evt(struct hci_dev *hdev,
					 struct sk_buff *skb)
{
...
static void klpr_hci_loglink_complete_evt(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	/* Create AMP hchan */
	hchan = (*klpe_hci_chan_create)(hcon);
	if (!hchan)
		return;
...
static void klpr_hci_disconn_loglink_complete_evt(struct hci_dev *hdev,
					     struct sk_buff *skb)
{
...
void klpp_hci_event_packet(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	case HCI_EV_USER_CONFIRM_REQUEST:
		klpp_hci_user_confirm_request_evt(hdev, skb);
		break;
	...
	case HCI_EV_LOGICAL_LINK_COMPLETE:
		klpr_hci_loglink_complete_evt(hdev, skb);
		break;

	case HCI_EV_DISCONN_LOGICAL_LINK_COMPLETE:
		klpr_hci_disconn_loglink_complete_evt(hdev, skb);
		break;
...
static struct klp_kallsyms_reloc klp_funcs[] = {
	...
	{ "hci_chan_create", (void *)&klpe_hci_chan_create, "bluetooth" },
	...
	{ "hci_cmd_complete_evt", (void *)&klpe_hci_cmd_complete_evt,
	  "bluetooth" },
	{ "hci_cmd_status_evt", (void *)&klpe_hci_cmd_status_evt,
	  "bluetooth" },
	{ "hci_conn_add", (void *)&klpe_hci_conn_add, "bluetooth" },
	{ "hci_conn_add_sysfs", (void *)&klpe_hci_conn_add_sysfs,
	  "bluetooth" },
...
int livepatch_bsc1240840_init(void)
{
	int ret;
...
void livepatch_bsc1240840_cleanup(void)
{

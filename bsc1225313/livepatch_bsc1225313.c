...
void klpe_amdgpu_gart_bind(struct amdgpu_device *adev, uint64_t offset,
		      int pages, dma_addr_t *dma_addr, uint64_t flags)
				KLP_RELOC_SYMBOL(amdgpu, amdgpu, amdgpu_gart_bind);
...
/* klp-ccp: from drivers/gpu/drm/amd/amdgpu/amdgpu_hmm.h */
#include <linux/types.h>
...
static void klpr_amdgpu_ttm_gart_bind_gfx9_mqd(struct amdgpu_device *adev,
				 ...
				 uint64_t flags)
{
	...
	if (abo->flags & AMDGPU_GEM_CREATE_CP_MQD_GFX9) {
		klpr_amdgpu_ttm_gart_bind_gfx9_mqd(adev, ttm, flags);
	} else {

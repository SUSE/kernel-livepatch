...
static int (*klpe_shm_try_destroy_orphaned)(int id, void *p, void *data);
...
void klpp_shm_destroy_orphaned(struct ipc_namespace *ns)
{
	...
	if (shm_ids(ns).in_use) {
		rcu_read_lock();
		idr_for_each(&shm_ids(ns).ipcs_idr, &(*klpe_shm_try_destroy_orphaned), ns);
		rcu_read_unlock();
	}
...
static struct klp_kallsyms_reloc klp_funcs[] = {
	{ "shm_try_destroy_orphaned", (void *)&klpe_shm_try_destroy_orphaned },
};

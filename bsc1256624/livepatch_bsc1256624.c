...
static int (*klpe_calipso_pad_write)(unsigned char *buf, unsigned int offset,
			     unsigned int count);

static int klpr_calipso_genopt(unsigned char *buf, u32 start, u32 buf_len,
			  ...
			  const struct netlbl_lsm_secattr *secattr)
{
	...
	(*klpe_calipso_pad_write)(buf, start, pad);
...
static int (*klpe_calipso_opt_find)(struct ipv6_opt_hdr *hop, unsigned int *start,
			    unsigned int *end);
				  ...
				  const struct netlbl_lsm_secattr *secattr)
{
	...
	if (ip6_hdr->nexthdr == NEXTHDR_HOP) {
		...
		ret_val = (*klpe_calipso_opt_find)(hop, &start, &end);
	...
	ret_val = klpr_calipso_genopt(buf, start & 3, sizeof(buf), doi_def, secattr);
	if (ret_val < 0)
		return ret_val;
	...
	(*klpe_calipso_pad_write)((unsigned char *)hop, new_end, pad);
...
static struct klp_kallsyms_reloc klp_funcs[] = {
	{ "calipso_opt_find", (void *)&klpe_calipso_opt_find },
	{ "calipso_pad_write", (void *)&klpe_calipso_pad_write },
};

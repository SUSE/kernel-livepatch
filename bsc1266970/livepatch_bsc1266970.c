...
static void (*klpe_kvm_mmu_audit)(struct kvm_vcpu *vcpu, int point);
...
static int (*klpe_ept_walk_addr_generic)(struct guest_walker *walker,
				    struct kvm_vcpu *vcpu, struct kvm_mmu *mmu,
				    gva_t addr, u32 access);

static int klpr_ept_walk_addr(struct guest_walker *walker,
			    struct kvm_vcpu *vcpu, gva_t addr, u32 access)
{
	return (*klpe_ept_walk_addr_generic)(walker, vcpu, &vcpu->arch.mmu, addr,
					access);
}
...
static int klpp_ept_fetch(struct kvm_vcpu *vcpu, gva_t addr,
...
static bool
klpr_ept_is_self_change_mapping(struct kvm_vcpu *vcpu,
			     ...
			     bool prefault)
{
	 ...
	 */
	r = klpr_ept_walk_addr(&walker, vcpu, addr, error_code);
	...
	is_self_change_mapping = klpr_ept_is_self_change_mapping(vcpu,
	...
	(*klpe_kvm_mmu_audit)(vcpu, AUDIT_PRE_PAGE_FAULT);
	if (klpr_make_mmu_pages_available(vcpu) < 0)
		goto out_unlock;
	if (!force_pt_level)
		(*klpe_transparent_hugepage_adjust)(vcpu, walker.gfn, &pfn, &level);
	r = klpp_ept_fetch(vcpu, addr, &walker, write_fault,
			 level, pfn, map_writable, prefault, lpage_disallowed);
	(*klpe_kvm_mmu_audit)(vcpu, AUDIT_POST_PAGE_FAULT);
...
static int (*klpe_paging64_walk_addr_generic)(struct guest_walker *walker,
				    struct kvm_vcpu *vcpu, struct kvm_mmu *mmu,
				    gva_t addr, u32 access);

static int klpr_paging64_walk_addr(struct guest_walker *walker,
			    struct kvm_vcpu *vcpu, gva_t addr, u32 access)
{
	return (*klpe_paging64_walk_addr_generic)(walker, vcpu, &vcpu->arch.mmu, addr,
					access);
}
...
static int klpp_paging64_fetch(struct kvm_vcpu *vcpu, gva_t addr,
			 struct guest_walker *gw,
			 int write_fault, int hlevel,
			 kvm_pfn_t pfn, bool map_writable, bool prefault,
			 bool lpage_disallowed)
...
static bool
klpr_paging64_is_self_change_mapping(struct kvm_vcpu *vcpu,
			      struct guest_walker *walker, int user_fault,
			      bool *write_fault_to_shadow_pgtable)
{
	int level;
			     ...
			     bool prefault)
{
	 ...
	 * Look up the guest pte for the faulting address.
	 */
	r = klpr_paging64_walk_addr(&walker, vcpu, addr, error_code);
	...
	is_self_change_mapping = klpr_paging64_is_self_change_mapping(vcpu,
	      &walker, user_fault, &vcpu->arch.write_fault_to_shadow_pgtable);
	...
	if (!force_pt_level)
		(*klpe_transparent_hugepage_adjust)(vcpu, walker.gfn, &pfn, &level);
	r = klpp_paging64_fetch(vcpu, addr, &walker, write_fault,
			 level, pfn, map_writable, prefault, lpage_disallowed);
	(*klpe_kvm_mmu_audit)(vcpu, AUDIT_POST_PAGE_FAULT);
...
static int (*klpe_paging32_walk_addr_generic)(struct guest_walker *walker,
				    struct kvm_vcpu *vcpu, struct kvm_mmu *mmu,
				    gva_t addr, u32 access);

static int klpr_paging32_walk_addr(struct guest_walker *walker,
			    struct kvm_vcpu *vcpu, gva_t addr, u32 access)
{
	return (*klpe_paging32_walk_addr_generic)(walker, vcpu, &vcpu->arch.mmu, addr,
					access);
}
...
static int klpp_paging32_fetch(struct kvm_vcpu *vcpu, gva_t addr,
			 struct guest_walker *gw,
			 int write_fault, int hlevel,
			 kvm_pfn_t pfn, bool map_writable, bool prefault,
			 bool lpage_disallowed)
...
static bool
klpr_paging32_is_self_change_mapping(struct kvm_vcpu *vcpu,
			      struct guest_walker *walker, int user_fault,
			      bool *write_fault_to_shadow_pgtable)
{
	int level;
			     ...
			     bool prefault)
{
	 ...
	 * Look up the guest pte for the faulting address.
	 */
	r = klpr_paging32_walk_addr(&walker, vcpu, addr, error_code);
	...
	is_self_change_mapping = klpr_paging32_is_self_change_mapping(vcpu,
	      &walker, user_fault, &vcpu->arch.write_fault_to_shadow_pgtable);
	...
	if (!force_pt_level)
		(*klpe_transparent_hugepage_adjust)(vcpu, walker.gfn, &pfn, &level);
	r = klpp_paging32_fetch(vcpu, addr, &walker, write_fault,
			 level, pfn, map_writable, prefault, lpage_disallowed);
	(*klpe_kvm_mmu_audit)(vcpu, AUDIT_POST_PAGE_FAULT);
...
static struct klp_kallsyms_reloc klp_funcs[] = {
	...
	{ "kvm_mmu_prepare_zap_page", (void *)&klpe_kvm_mmu_prepare_zap_page,
	  "kvm" },
	{ "kvm_release_pfn_clean", (void *)&klpe_kvm_release_pfn_clean,
	  "kvm" },
	...
	{ "paging32_walk_addr_generic",
	  (void *)&klpe_paging32_walk_addr_generic, "kvm" },
	...
	{ "paging64_walk_addr_generic",
	  (void *)&klpe_paging64_walk_addr_generic, "kvm" },

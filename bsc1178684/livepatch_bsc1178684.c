...
static sense_reason_t	(*klpe_transport_generic_map_mem_to_cmd)(struct se_cmd *,
...
static sense_reason_t (*klpe_target_setup_cmd_from_cdb)(struct se_cmd *, unsigned char *);
...
static int klpr_target_xcopy_setup_pt_cmd(
	struct xcopy_pt_cmd *xpt_cmd,
	struct xcopy_op *xop,
	...
	bool alloc_mem)
{
	...
	cmd->tag = 0;
	sense_rc = (*klpe_target_setup_cmd_from_cdb)(cmd, cdb);
	if (sense_rc) {
		ret = -EINVAL;
		goto out;
	}
	...
	if (alloc_mem) {
		rc = (*klpe_target_alloc_sgl)(&cmd->t_data_sg, &cmd->t_data_nents,
		 ...
		 */
		sense_rc = (*klpe_transport_generic_map_mem_to_cmd)(cmd,
	...
	u32 src_sectors)
{
	...
	rc = klpr_target_xcopy_setup_pt_cmd(xpt_cmd, xop, src_dev, &cdb[0],
				remote_port, true);
	if (rc < 0) {
	...
	u32 dst_sectors)
{
	...
	rc = klpr_target_xcopy_setup_pt_cmd(xpt_cmd, xop, dst_dev, &cdb[0],
				remote_port, false);
	if (rc < 0) {
...
void klpp_target_xcopy_do_work(struct work_struct *work)
{
	...
	while (src_lba < end_lba) {
		...
		rc = klpr_target_xcopy_write_destination(ec_cmd, xop, dst_dev,
						dst_lba, cur_nolb);
		if (rc < 0) {
			(*klpe_transport_generic_free_cmd)(&xop->src_pt_cmd->se_cmd, 0);
			goto out;
		}
		...
		(*klpe_transport_generic_free_cmd)(&xop->dst_pt_cmd->se_cmd, 0);
	}
...
static struct klp_kallsyms_reloc klp_funcs[] = {
	...
	{ "transport_generic_map_mem_to_cmd",
	  (void *)&klpe_transport_generic_map_mem_to_cmd, "target_core_mod" },
	{ "transport_init_se_cmd", (void *)&klpe_transport_init_se_cmd,
	  "target_core_mod" },
	{ "target_setup_cmd_from_cdb", (void *)&klpe_target_setup_cmd_from_cdb,
	  "target_core_mod" },

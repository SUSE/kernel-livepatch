...
static __poll_t (*klpe_ep_read_events_proc)(struct eventpoll *ep, struct list_head *head,
			       void *priv);
static void (*klpe_ep_ptable_queue_proc)(struct file *file, wait_queue_head_t *whead,
				 poll_table *pt);

static __poll_t klpr_ep_item_poll(const struct epitem *epi, poll_table *pt,
				 int depth)
{
	struct eventpoll *ep;
	bool locked;
...
static int klpr_reverse_path_check(void)
{
	...
	list_for_each_entry(current_file, &(*klpe_tfile_check_list), f_tfile_llink) {
		...
		error = ep_call_nested(&(*klpe_poll_loop_ncalls),
...
static int klpr_ep_insert(struct eventpoll *ep, const struct epoll_event *event,
...
static int klpr_ep_modify(struct eventpoll *ep, struct epitem *epi,
...
static int klpr_ep_loop_check(struct eventpoll *ep, struct file *file)
{
	...
	ret = ep_call_nested(&(*klpe_poll_loop_ncalls),

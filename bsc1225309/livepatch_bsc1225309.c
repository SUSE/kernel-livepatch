...
static void (*klpe_cifs_put_tlink)(struct tcon_link *tlink);
...
static void (*klpe_cifs_queue_oplock_break)(struct cifsFileInfo *cfile);
...
static void (*klpe_smb2_cached_lease_break)(struct work_struct *work);
...
/* klp-ccp: from fs/cifs/smb2misc.c */
static const __le16 (*klpe_smb2_rsp_struct_sizes)[NUMBER_OF_SMB2_COMMANDS];
...
static void
(*klpe_cifs_ses_oplock_break)(struct work_struct *work);

#include "livepatch_bsc1225309.h"

static void
klpr_smb2_queue_pending_open_break(struct tcon_link *tlink, __u8 *lease_key,
			      __le32 new_lease_state)
{
	...
	if (!lw) {
		(*klpe_cifs_put_tlink)(tlink);
		return;
	}

	INIT_WORK(&lw->lease_break, (*klpe_cifs_ses_oplock_break));
	lw->tlink = tlink;
	lw->lease_state = new_lease_state;
	memcpy(lw->lease_key, lease_key, SMB2_LEASE_KEY_SIZE);
	queue_work((*klpe_cifsiod_wq), &lw->lease_break);
}

static bool
klpr_smb2_tcon_has_lease(struct cifs_tcon *tcon, struct smb2_lease_break *rsp)
{
	__u8 lease_state;
	...
	list_for_each_entry(cfile, &tcon->openFileList, tlist) {
		...
		(*klpe_cifs_queue_oplock_break)(cfile);
		return true;
	}
...
klpr_smb2_tcon_find_pending_open_lease(struct cifs_tcon *tcon,
...
static bool
klpr_smb2_is_valid_lease_break(char *buffer)
{
	...
	list_for_each_entry(server, &(*klpe_cifs_tcp_ses_list), tcp_ses_list) {
		list_for_each_entry(ses, &server->smb_ses_list, smb_ses_list) {
			...
			list_for_each_entry(tcon, &ses->tcon_list, tcon_list) {
				...
				if (klpr_smb2_tcon_has_lease(tcon, rsp)) {
					spin_unlock(&tcon->open_file_lock);
					spin_unlock(&(*klpe_cifs_tcp_ses_lock));
					return true;
				}
				open = klpr_smb2_tcon_find_pending_open_lease(tcon,
									 rsp);
				if (open) {
					...
					spin_unlock(&tcon->open_file_lock);
					spin_unlock(&(*klpe_cifs_tcp_ses_lock));
					klpr_smb2_queue_pending_open_break(tlink,
								      lease_key,
								      rsp->NewLeaseState);
					return true;
				}
				...
				if (tcon->crfid.is_valid &&
					...
					INIT_WORK(&tcon->crfid.lease_break,
						  (*klpe_smb2_cached_lease_break));
					queue_work((*klpe_cifsiod_wq),
...
klpp_smb2_is_valid_oplock_break(char *buffer, struct TCP_Server_Info *server)
{
	...
	if (rsp->StructureSize !=
				(*klpe_smb2_rsp_struct_sizes)[SMB2_OPLOCK_BREAK_HE]) {
		if (le16_to_cpu(rsp->StructureSize) == 44)
			return klpr_smb2_is_valid_lease_break(buffer);
		else
			return false;
	}
...
static struct klp_kallsyms_reloc klp_funcs[] = {
	...
	{ "cifs_put_tlink", (void *)&klpe_cifs_put_tlink, "cifs" },
	{ "cifs_queue_oplock_break", (void *)&klpe_cifs_queue_oplock_break,
	  "cifs" },
	{ "cifs_ses_oplock_break", (void *)&klpe_cifs_ses_oplock_break,
	  "cifs" },
	...
	{ "smb2_cached_lease_break", (void *)&klpe_smb2_cached_lease_break,
	  "cifs" },
	{ "smb2_rsp_struct_sizes", (void *)&klpe_smb2_rsp_struct_sizes,
	  "cifs" },
};

...
static int (*klpe_cifs_ipaddr_cmp)(struct sockaddr *srcaddr, struct sockaddr *rhs);
...
/* klp-ccp: from fs/smb/client/smb2ops.c */
static int
klpr_iface_cmp(struct cifs_server_iface *a, struct cifs_server_iface *b)
{
	int cmp_ret = 0;
	...
	if (a->rdma_capable == b->rdma_capable) {
		if (a->rss_capable == b->rss_capable) {
			if (a->speed == b->speed) {
				cmp_ret = (*klpe_cifs_ipaddr_cmp)((struct sockaddr *) &a->sockaddr,
			...
			size_t buf_len, struct cifs_ses *ses, bool in_mount)
{
	...
	while (bytes_left >= (ssize_t)sizeof(*p)) {
		...
		list_for_each_entry_safe(iface, niface, &ses->iface_list,
					 iface_head) {
			ret = klpr_iface_cmp(iface, &tmp_iface);
			if (!ret) {
...
static struct klp_kallsyms_reloc klp_funcs[] = {
	...
	{ "cifs_ipaddr_cmp", (void *)&klpe_cifs_ipaddr_cmp, "cifs" },
};

...
static struct nfc_llcp_sock *(*klpe_nfc_llcp_sock_get)(struct nfc_llcp_local *local,
					       u8 ssap, u8 dsap);
...
static struct nfc_llcp_sock *(*klpe_nfc_llcp_sock_from_sn)(struct nfc_llcp_local *local, const u8 *sn, size_t sn_len, bool needref);

static struct nfc_llcp_sock *klpr_nfc_llcp_sock_get_sn(struct nfc_llcp_local *local,
						  u8 *sn, size_t sn_len)
{
	return (*klpe_nfc_llcp_sock_from_sn)(local, sn, sn_len, true);
}
			     ...
			     struct sk_buff *skb)
{
	...
	llcp_sock = (*klpe_nfc_llcp_sock_get)(local, dsap, LLCP_SAP_SDP);
				  ...
				  struct sk_buff *skb)
{
	...
	if (dsap != LLCP_SAP_SDP) {
		sock = (*klpe_nfc_llcp_sock_get)(local, dsap, LLCP_SAP_SDP);
			       ...
			       struct sk_buff *skb)
{
	...
	llcp_sock = (*klpe_nfc_llcp_sock_get)(local, dsap, ssap);
	if (llcp_sock == NULL) {
			      ...
			      struct sk_buff *skb)
{
	...
	while (offset < tlv_len) {
		...
		case LLCP_TLV_SDREQ:
			...
			llcp_sock = (*klpe_nfc_llcp_sock_from_sn)(local, service_name,
							  service_name_len,
							  true);
			if (!llcp_sock) {
				sap = 0;
...
static struct klp_kallsyms_reloc klp_funcs[] = {
	...
	{ "nfc_llcp_sock_from_sn", (void *)&klpe_nfc_llcp_sock_from_sn,
	  "nfc" },
	{ "nfc_llcp_sock_get", (void *)&klpe_nfc_llcp_sock_get, "nfc" },
